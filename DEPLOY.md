...
3. 「運営の方：チームを登録して招待コードを作る」を開く
4. チーム名と **GitHubリポジトリ**（`owner/repository` 形式）を入力

ここで入れるリポジトリ名が、Webhookの送信元と**完全一致**している必要があります。ここがずれていると、200が返るのに画面が動きません。
...
- [ ] 招待コードでチームに参加でき、`/dashboard` に移動する
