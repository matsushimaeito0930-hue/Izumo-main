...
`vercel.json` で関数の実行先を東京（`hnd1`）にしています。Supabaseを東京（`ap-northeast-1`）で作っている前提です。

既定のままだとVercelはワシントン（`iad1`）で動き、**DBへの問い合わせが毎回太平洋を往復**します。`/api/state` はSupabaseへ3回続けて問い合わせるので、往復のたびに遅延が積み上がり、pushしてから画面に出るまで数秒かかります。

Supabaseを別のリージョンで作った場合は、`vercel.json` の `regions` をそちらに合わせてください。プランの制限で弾かれる場合は、このファイルを消せば既定に戻ります。
