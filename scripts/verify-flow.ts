...
// push 1 + push 1 + PR作成 2 + PRマージ 5 = 9
check("Team Alpha のスコア", alpha?.score, 9);
...
check("花子の点数（push1 + マージ5）", hanakoRow?.score, 6);
...
check("上限超えも既定値に戻る", rejected.config.pull_request_merged, 5);
