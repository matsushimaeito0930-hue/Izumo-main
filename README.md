...
- 1つのGitHubアカウントで複数イベントを主催・参加でき、選択中のイベントごとにデータと役割を分離する。

### 現在実装されている機能

1. GitHub OAuthログイン
   - `GITHUB_CLIENT_ID` と `GITHUB_CLIENT_SECRET` がある場合に有効。
   - ログイン直後は参加者。イベントを作成した本人だけが、そのイベントの運営になる。
   - セッションは `hackverse-auth` というhttpOnly CookieにHS256 JWTで保存する。有効期限は12時間。
2. イベントとチームの登録
   - 運営がイベント名を登録・更新できる。
   - イベント名の変更ではチームや履歴を消さない。新しい開催回は新規イベントとして作成する。
...
8. 役割はイベント単位
   - 同じGitHubアカウントでも、イベントAでは運営、イベントBでは参加者として利用できる。
   - 運営権限は `events.owner_github_username` と選択中のイベントIDをAPI側で照合する。
   - 参加者・メンター・運営のDM候補も `event_members` でイベント内だけに限定する。
9. 所属のやり直し
   ...
   - 確認欄にコードそのものは表示しない。表示すると写すだけで済み、確認にならないため。運営は上の「招待コード（イベント共通）」まで戻って自分で確かめる。
   - 削除されるのはそのイベントのチーム・所属・招待コード・活動履歴・質問・チャットだけ。他のイベントのデータや、ユーザー登録そのものには影響しない。
10. 運営の質問対応
...
| 運営 | 必要 | イベント作成・チーム編集・削除などの管理操作を本人に限定する |
...
| `tests/event-scoring.test.ts` | イベント別配点、再計算、同一リポジトリの複数イベント反映 |
| `tests/help-privacy.test.ts` | 匿名投稿の投稿者情報をAPIレスポンスから隠すこと |
...
- `events`: 複数イベント、所有者、全体コード、配点（`score_config`）
- `users`: GitHubユーザーの共通プロフィール
- `event_members`: イベントごとの役割とメンターの得意分野
...
npm.cmd run build
npm.cmd run verify:flow  # 65/65
npm.cmd run verify:core  # 67/67
```

Vitestは17ファイル・60件が成功し、`npm audit`は0件です。
...
- 運営操作はイベント所有者のGitHubアカウントだけに許可し、固定allowlistには戻さない。
- イベント削除は所属・履歴を含む破壊的操作なので、招待コードの再入力による確認導線を維持する。
...
- DBを変更するときは、既存DBに移行SQLを適用・検証してからアプリをデプロイする。移行前のDBでWebhookを旧方式にフォールバックさせない。
...
3. 新規DBは`supabase/schema.sql`、既存DBは`DEPLOY.md`に記載した移行手順を適用する
...
| 操作 | そのイベントでの役割 |
| --- | --- |
| GitHubログイン後にイベントを作成 | 運営 |
| イベントコードと部屋番号で参加 | 参加者 |
| 招待コードでメンター登録 | メンター |
| 審査員として参加 | 審査員 |

ロールは `participant`、`mentor`、`admin`、`judge` の4種類です。役割は
`event_members` と署名済みセッションにイベントIDと一緒に保持するため、同じ人が
イベントAでは運営、イベントBでは参加者になれます。
...
運営権限は画面の自己申告ではなく、選択中イベントの所有者とGitHubログイン名をAPIで照合します。
`ADMIN_GITHUB_LOGINS` は廃止済みです。
...
通常の参加フォームでは、取り違え防止のため**2つのコード**を使います。
...
招待URLなら2つのコードが自動入力されます。部屋番号は全体で一意なので、部屋番号だけの参加経路も利用できます。
メンターは招待コードだけで登録します（チームに属さないため）。

**なぜ2つに分けているか。** ハッカソンを複数同時に動かすと、
部屋番号だけだと別イベントの部屋に入れてしまいます。招待コードで先にイベントを絞ることで、
部屋番号がイベントをまたいで衝突しても安全に運用できます。

### 複数イベント

イベント、チーム、配点、活動、質問、チャット、DM候補、WakaTime集計は選択中のイベントで分離します。
同じGitHubリポジトリを複数イベントで再利用した場合は、1回のWebhookを該当する各イベントへ反映します。
同一イベント内では1アカウント1チームですが、別イベントへの同時参加は可能です。

## 運営の画面と参加者の画面

同じURLですが、現在選択しているイベントでの役割によって中身が変わります。
...
イベントは複数同時に運用できます。イベントコードと部屋番号の組が違う参加はAPI側で拒否されます。
同じGitHubアカウントは、同一イベント内の別チームには参加できませんが、別イベントには参加できます。
イベント名を変更してもチームと進捗は保持されます。新しい開催回は新規イベントとして作成してください。

## Supabaseのセットアップ

1. Supabaseプロジェクトを作成する
2. SQLエディタを開く
3. 新規DBは`supabase/schema.sql`を実行する。既存DBはバックアップ後に`supabase/migrations/20260913_event_isolation_and_webhook_atomic.sql`を適用する（詳細は`DEPLOY.md`）
...
- Supabaseではスコア更新、活動追加、Webhook重複排除をDB関数の1トランザクションで処理します。移行SQLが未適用なら記録を失敗として返し、誤った点数を確定させません。
...
本番では `ENABLE_DEMO_MODE=false`、Supabase・GitHub OAuth・署名用secretの設定が必須です。
