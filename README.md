...
未設定の場合は従来どおり、表示名を手入力して参加するフローで動きます（ローカルデモ用）。
...
2. チーム名とGitHubリポジトリ（`owner/repository` 形式）を入力する
3. HackVerse が招待コードを発行する
4. 参加者がGitHubでログインし、招待コードを入力する
5. 参加者は `/dashboard` に移動する
6. GitHubリポジトリはすでにそのチームへ紐づいており、Webhookの更新が反映される
