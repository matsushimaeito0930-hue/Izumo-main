...
9. 所属のやり直し
   ...
   - イベントごと削除もできる（運営セクションの最下部）。取り返しがつかないので、イベント名を打ち直すまでボタンが押せない二段構えにしている。APIも `confirmName` の一致を確認するので、直接叩いても素通りしない。
...
| `DELETE` | `/api/admin/event` | イベント削除（`confirmName` にイベント名の一致が必要） |
